...
def createFile(dest):
	...
	name = "%d_%d_%d.html" % (date[1], date[2], (date[0] % 100)) #output in month_date_year.html format
	if not (path.isfile(dest + name)): #checks if file already exists.  If not, then create file
		filenew = open(dest + name, "w")
		names = ["Raymond","James","Michelle","Dad","Mom"]
		filenew.write("I write some text saved as an .html file.  If I want to save as a text file, change .html to .txt in name = \"%d_%d_%d.html\" % (date[1], date[2], (date[0] % 100))")
		filenew.write("\n")
		for eachname in names:
			filenew.write(eachname+ " in the Mar family.")
			filenew.write("\n")
		filenew.close()
